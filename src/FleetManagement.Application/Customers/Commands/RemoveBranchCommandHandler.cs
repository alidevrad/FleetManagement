...
public class RemoveBranchCommandHandler : IRequestHandler<RemoveBranchCommand>
{
    private readonly ICustomerCommandRepository _customerRepository;
    private readonly ICustomerQueryRepository _customerQueryRepository;


    public RemoveBranchCommandHandler(ICustomerCommandRepository customerRepository,
    ...
    public async Task Handle(RemoveBranchCommand request, CancellationToken cancellationToken)
    {
        var customer = await _customerQueryRepository.GetByIdAsync(request.CustomerId);
        if (customer == null)
            throw new InvalidOperationException("Customer not found.");

        customer.RemoveBranch(request.BranchId);
